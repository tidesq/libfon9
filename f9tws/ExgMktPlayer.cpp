...
class ExgMktPlayer : public ExgMktFeeder, public fon9::io::Session {
   ...
   void ReadMktFile() {
      if (this->Device_->OpImpl_GetState() != fon9::io::State::LinkReady)
         return;
      auto node = fon9::FwdBufferNode::Alloc(1024 * 4);
      auto rdres = this->MktFile_.Read(this->ReadFrom_, node->GetDataEnd(), node->GetRemainSize());
      if (rdres.HasResult() && rdres.GetResult() > 0) {
         this->ReadFrom_ += rdres.GetResult();
         ...
         if (now - this->LastStTime_ >= fon9::TimeInterval_Second(1)) {
            ...
            std::string stmsg = fon9::RevPrintTo<std::string>("Pk=", this->SentPkCount_, "|Pos=", this->ReadFrom_);
         ...
         fon9::FreeNode(node);
         std::string stmsg = fon9::RevPrintTo<std::string>("Pk=", this->SentPkCount_, "|Pos=", this->ReadFrom_, "|Rd=", rdres);
         this->Device_->Manager_->OnSession_StateUpdated(*this->Device_, &stmsg, fon9::LogLevel::Info);
      }
   }
public:
   ExgMktPlayer(fon9::File&& fd) : MktFile_{std::move(fd)} {
   }
   ~ExgMktPlayer() {
   }
};
...
public:
   ...
   static fon9::io::SessionSP CreateSession(fon9::StrView cfg, std::string& errReason) {
      // TODO: cfg.SessionArgs_: fileName|from|to|speed|
      // std::string ExgMktPlayer::SessionCommand(Device& dev, StrView cmdln);
      //    - pause
      //    - restart [from to speed]
      fon9::File fd;
      auto       res = fd.Open(cfg.ToString(), fon9::FileMode::Read);
      if (!res) {
         errReason = fon9::RevPrintTo<std::string>("Create:TwsExgMktPlayer|fname=", fd.GetOpenName(), '|', res);
         return fon9::io::SessionSP{};
      }
      return fon9::io::SessionSP{new ExgMktPlayer{std::move(fd)}};
   }
