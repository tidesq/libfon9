...
void IocpSocket::OnIocp_Error(OVERLAPPED* lpOverlapped, DWORD eno) {
   fon9_LOG_DEBUG("IocpSocket.OnIocp_Error|err=", this->GetOverlappedKind(lpOverlapped), ':', GetSysErrC(eno));
   if (this->Eno_ == 0)
      this->Eno_ = eno;
...
void IocpSocket::OnIocp_Done(OVERLAPPED* lpOverlapped, DWORD bytesTransfered) {
   //fon9_LOG_DEBUG("IocpSocket.OnIocp_Done|", this->GetOverlappedKind(lpOverlapped),
