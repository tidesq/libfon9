...
private:
   ...
   uint32_t    NextChunkSize_{0};
   uint32_t    CurrChunkFrom_{0};
   ...
   struct Compare {
      std::string* OrigStr_;
      bool operator()(const StrVref& a, const StrVref& b) const {
         return icompare(a.ToStrView(this->OrigStr_->c_str()), b.ToStrView(this->OrigStr_->c_str())) < 0;
      }
      bool operator()(const StrView& a, const StrVref& b) const {
         return icompare(a, b.ToStrView(this->OrigStr_->c_str())) < 0;
      }
      bool operator()(const StrVref& a, const StrView& b) const {
         return icompare(a.ToStrView(this->OrigStr_->c_str()), b) < 0;
      }
   };
   using Fields = SortedVector<StrVref, FieldValue, Compare>;
